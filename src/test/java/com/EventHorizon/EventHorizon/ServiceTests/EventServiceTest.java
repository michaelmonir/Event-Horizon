...
public class EventServiceTest {
    ...
    public void gettingEventForUser() {
        ...
        Assertions.assertEquals(viewEventDTO, expectedViewEventDto);
    }
    ...
    public void gettingOrganizerEvent() {
        ...
        Assertions.assertEquals(expectedViewEventDTO, detailedEventDTO);
    }
    ...
    public void createEvent() {
        ...
        Assertions.assertEquals(detailedEventDTO, resultEventDTO);
    }
    ...
    private void initializeEventRepositoryServiceMockito() {
        Mockito.when(this.eventRepositoryService.getEventAndHandleNotFound(Mockito.any(int.class))).thenReturn(this.customEvent);
        Mockito.when(this.eventRepositoryService.saveEventWhenCreatingAndHandleAlreadyExisting(Mockito.any(Event.class))).thenReturn(this.customEvent);
        Mockito.when(this.eventRepositoryService.updateEventAndHandleNotFound(Mockito.any(Event.class))).thenReturn(this.customEvent);
    }

    private void initializeDetailedEventDtoMapper() {
        Mockito.when(this.delaitedEventDtoMapper.getDTOfromDetailedEvent(Mockito.any(Event.class))).thenReturn(this.customDetailedEventDto);
        Mockito.when(this.delaitedEventDtoMapper.getDTOfromDetailedEvent(Mockito.any(Event.class))).thenReturn(this.customDetailedEventDto);
        Mockito.when(this.delaitedEventDtoMapper.getEventFromDetailedEventDTO(Mockito.any(DetailedEventDto.class))).thenReturn(this.customEvent);
    }

    private void initializeEventDtoMapper() {
        Mockito.when(this.viewEventDtoMapper.getEventFromViewEventDTO(Mockito.any(ViewEventDto.class))).thenReturn(this.customEvent);
        Mockito.when(this.viewEventDtoMapper.getDTOfromViewEvent(Mockito.any(Event.class))).thenReturn(this.customViewEventDto);
    }

    private void initializerInformationService() {
        Mockito.when(this.informationService.getByID(Mockito.any(int.class))).thenReturn(this.customInformation);
    }

    private void initializeOrganizerInformationService() {
        Mockito.when(this.organizerInformationService.getUserByInformation(Mockito.any(Information.class))).thenReturn(this.customOrganizer);
    }
    ...
    private void initializeCustomEvent() {
        this.customEvent = Event.builder().id(1).name("My Event").eventLocation(new Location()).eventAds(new AdsOption()).build();
    }

    private void initializeCustomInformation() {
        this.customInformation = Information.builder().id(1).build();
    }

    private void initializeCustomOrganizer() {
        this.customOrganizer = Organizer.builder().id(1).information(customInformation).build();
    }

    private void initializeViewEventDTO() {
        this.customViewEventDto = ViewEventDto.builder().id(1).name("My Event").build();
        this.customViewEventDto.setEventLocation(new Location());
    }

    private void initializeDetailedEventDTO() {
        this.customDetailedEventDto = DetailedEventDto.builder().id(1).name("My Event").eventLocation(new Location()).eventAds(new AdsOptionDto()).build();
    }
}
